...
function Public({onAdmin}){
    ...
    return <div className="site">
        ...
        <main id="top">
            ...
            <section id="gallery" className="gallery-section"><div className="section-head"><div><div className="section-label">NOSSO UNIVERSO</div><h2>Inspire-se<br/><span>na garagem.</span></h2></div></div>{data.gallery.length===0?<p className="empty-gallery">Galeria em atualização.</p>:<div className="gallery-grid">{data.gallery.map(x=><figure key={x.id} data-testid={`gallery-item-${x.id}`}><img src={x.image_url} alt={x.title} loading="lazy" style={{objectPosition:x.crop==='top'?'center top':x.crop==='bottom'?'center bottom':'center'}}/><figcaption><small>{x.category}</small><b>{x.title}</b></figcaption></figure>)}</div>}</section>
